...
def reformat(dataset, labels):
    dataset = dataset.reshape((-1, IMAGE_SIZE, IMAGE_SIZE, NUM_CHANNELS*NUM_FRAMES)).astype(np.float32)
...
with graph.as_default():
  ...
  # Input data.
    tf_train_dataset = tf.placeholder(tf.float32, shape=(BATCH_SIZE, IMAGE_SIZE, IMAGE_SIZE, NUM_CHANNELS*NUM_FRAMES))
    tf_train_labels = tf.placeholder(tf.float32, shape=(BATCH_SIZE, NUM_CLASSES))
    tf_valid_dataset = tf.constant(valid_dataset)
    keep_prob=tf.placeholder(tf.float32)

    # Variables.
    layer1_weights = tf.get_variable("layer1_weights",shape=[PATCH_SIZE1, PATCH_SIZE1, NUM_CHANNELS*NUM_FRAMES, DEPTH1], initializer=tf.contrib.layers.xavier_initializer())
    ...
    layer4_weights = tf.get_variable("layer4_weights",shape=[IMAGE_SIZE // 4 * IMAGE_SIZE // 4 * DEPTH3, NUM_HIDDEN], initializer=tf.contrib.layers.xavier_initializer())
...
with tf.Session(graph=graph, config=config) as session:
    ...
    for step in range(NUM_STEPS):
        ...
        #Pause after n num steps and save weights
...
g = tf.Graph()
with g.as_default():
    x_input = tf.placeholder(tf.float32, shape=[1,IMAGE_SIZE,IMAGE_SIZE,NUM_CHANNELS*NUM_FRAMES], name="input")
    ...
    FC1 = tf.reshape(MAXPOOL3, [shape[0], shape[1] * shape[2] * shape[3]])
